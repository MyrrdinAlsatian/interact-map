...
- Q: What exact schema-version compatibility window should validators enforce? → A: Accept only `current` and `current-1`; reject `current-2` and older.
- Q: What exact metric semantics should the baseline observability contract require? → A: `request_latency_ms` records every HTTP response duration in milliseconds, `fragment_error_count` increments on each missing-fragment `422`, and `parse_error_count` increments on each parser validation failure.
...
- Missing Unpoly fragment target returns `422` with structured error payload, after which the client performs a full-page `GET` to the originating target URL.
...
- Parser accepts only `current` and `current-1` `schemaVersion` values; `current-2` and older versions are rejected with structured compatibility errors.
...
- **FR-002**: System MUST use Unpoly for progressive enhancement of navigation and fragment updates.
...
- **FR-009**: System MUST require authentication for application access, allow read operations for authenticated `viewer`, `security`, `editor`, and `admin` roles, allow standard writes/imports for `editor`, allow security and audit oversight operations for `security`, and reserve privileged administration operations for `admin`.
...
- **FR-011**: System MUST return `422` structured fragment errors for missing Unpoly targets, after which the client MUST perform a full-page `GET` to the originating target URL.
- **FR-012**: System MUST emit structured JSON logs and core metrics for backend and capability-island interactions, where `request_latency_ms` records every HTTP response duration in milliseconds, `fragment_error_count` increments on each missing-fragment `422`, and `parse_error_count` increments on each parser validation failure.
...
- **FR-014**: System MUST support only `current` and `current-1` `schemaVersion` values for graph/parser contracts and MUST reject `current-2` and older versions with structured compatibility errors.
- **FR-015**: System MUST provide an accessibility baseline using semantic HTML, readable text contrast, and keyboard-reachable primary interactions for server-rendered pages and capability-island controls.
...
- **UserRole**: Access-control role set of `viewer`, `security`, `editor`, and `admin` used to govern read access, standard mutations, security oversight, and privileged administration.
